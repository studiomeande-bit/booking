...
(function () {
  ...
  var T = {
    ...
    isMail:     { ko: '이 세션은 📮 우편 수령으로 진행됩니다. 인화물 발송이 완료되면 메일로 안내드립니다. 추가 비용이 있는 경우 입금 확인 후 발송됩니다. 픽업으로 변경을 원하시면 스튜디오로 연락해 주세요.', en: 'This session is set for 📮 postal delivery. We will email you once your prints are shipped. If there are extra charges, we ship once the payment is confirmed. Contact us if you prefer pickup instead.', de: 'Diese Sitzung läuft über 📮 Postversand. Wir informieren Sie per E-Mail nach dem Versand. Bei Zusatzkosten erfolgt der Versand nach Zahlungseingang. Für Abholung kontaktieren Sie uns bitte.' },
    ...
    msCopy:     { ko: '직접 방문이 어려우시면 우편 수령으로 변경할 수 있습니다. 예약된 픽업이 있다면 자동으로 취소되며, 추가 비용이 있는 경우 입금 확인 후 발송됩니다.', en: 'If visiting the studio is difficult, you can switch to postal delivery. Any booked pickup will be cancelled automatically; if there are extra charges, we ship once the payment is confirmed.', de: 'Falls ein Besuch schwierig ist, können Sie auf Postversand umstellen. Ein gebuchter Abholtermin wird automatisch storniert; bei Zusatzkosten erfolgt der Versand nach Zahlungseingang.' },
