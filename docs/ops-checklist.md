...
node scripts/check-all.mjs --online          # 배포 전 게이트 전부(27개). 실패면 배포 금지 — --online 이어야 라이브 항목(내부 단가 노출·select 301)까지 본다
...
| **게이트 단일 러너 (배포 전 첫 명령)** | `node scripts/check-all.mjs` — `scripts/check-*.mjs` 26개 + `verify-release-gate.mjs --offline` 를 **동시에** 돌려 한 표로 보여주고, 하나라도 실패하면 exit 1. 전부 오프라인·읽기 전용이라 0.6초면 끝난다. 왜: 게이트가 늘어나는 동안 **아무도 전부 돌리지 않아서** 2026-09-23 점검 때 26개 중 5개가 빨간 채 몇 주 방치됐다(전부 하네스 드리프트 — Code.gs 버그 아님). 개별 검증기를 기억해 돌리는 방식은 이미 실패했다. 옵션: **`--online`**(릴리스 게이트의 라이브 항목까지 — 기본 오프라인은 `[Phase 1] 내부 단가 노출` 과 `select 루트 → /v2/ 301` 을 **건너뛴다**. 2026-09-25 적발: 건너뜀이 '통과' 로 계산돼 이 두 검사가 배포 흐름에서 한 번도 돌지 않았다. 지금은 건너뛴 사실이 요약에 찍힌다) · `--only <문자열>` · `--quiet` · **`--future N`**(시계를 N일 앞으로 밀어 실행 — 고정 날짜 픽스처가 상대 창에서 미끄러지는 부패를 미리 잡는다. 분기·월마감·`sinceDays` 픽스처를 건드렸으면 `--future 120` 도 같이). 러너 자체는 실패 검출을 실증해 두었다(실패·throw 게이트를 심어 exit 1·본문 출력 확인). |
