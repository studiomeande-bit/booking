...
- 잔여(오너): Apps Script 200버전 한도 정리 — **2026-09-25 현재 158/200**(`clasp list-versions` 실측). 남은 슬롯 42. 삭제는 소유자만 가능(Apps Script 편집기 → 프로젝트 기록). clasp 에는 버전 삭제 명령이 없다.
...
사장님 요청("픽업예약은 리마인더 없어?" → "만들어줘"). 종전 픽업 메일은 ① 인화 완료 안내 ② 픽업 확정(예약·변경 시) ③ C4 **미예약** 리마인드뿐이라, **예약해 둔 손님**의 노쇼를 막는 알림이 없었다(노쇼는 브리핑 '픽업노쇼' 로 사후에만 잡혔다).

- **동작**: 매일 08:00 `dailyTasks`(B2 촬영 리마인드와 같은 시각)에서 **내일** 픽업 예약자에게 3개국어 메일 — 일시(요일 포함) · 스튜디오 주소 · 남은 추가금(있을 때) · "일정 변경 / 우편 전환" 버튼. 휴무일에도 보낸다(화요일 픽업 → 월요일 안내).
- **안 보내는 경우**: 수령 완료 · 우편 전환 · 작업 마감/우편발송 상태 · 예약 취소(자동취소 포함) · 이메일 없음/수기 · 이미 이 약속에 알림. 새 셀렉 열 `픽업전날알림`(맨 끝, 인덱스 59)에 **알린 픽업일시** 를 적어 같은 약속엔 1회, 일정을 바꾸면 새 일정 전날 다시. 값은 `parseDateSafe_` 로 비교(시트가 Date 로 바꿔 저장해도 매일 재발송 안 함). 발송 직전 픽업일시·수령완료 재조회, 실행당 15건 상한, 메일 쿼터 < 30 이면 전체 보류.
- **배포 전 적대적 검토 확정 2건 반영**:
  - 🔴 **캘린더가 정본인데 시트만 봤다** — 사장님이 [픽업] 일정을 캘린더에서 직접 옮기거나 지우면 옛 시간이 안내되고, 일찍 찾아가 수령 처리(이벤트 삭제)된 뒤 재인화가 돌면 **이미 받아 간 손님에게 "내일 오세요"** 가 나갈 수 있었다(단계별 재현). → 내일 이벤트를 **한 번에** 읽어(`getEvents` — 지운 이벤트는 안 돌려준다) 이벤트가 있고 시작 시각이 시트와 같을 때만 보낸다. 다르면 보내지 않고 `held` 에 사유(삭제됨/시간 불일치/이벤트 없음/캘린더 확인 실패)를 남겨 운영 로그·dryRun 에 보인다. 캘린더를 못 읽으면 전원 보류(틀린 안내보다 안 보내는 게 낫다).
  - **인화 전 약속**(전화로 잡은 시간 등)은 픽업 페이지가 '시간 변경' 버튼을 숨긴다 → 메일도 "다른 시간을 고르실 수 있다" 고 쓰지 않고 답장·우편 전환만 안내.
- **에이전트 액션** `select-pickup-dayprior-run` `{"dryRun":true}` — 내일 대상·보류 목록. 문자열 `"true"` 도 dryRun 으로(메일 액션은 애매하면 안 보낸다). 운영 보드 작업명에 C7 추가, **빠져 있던 C6(우편발송 전 결제요청)도 추가**.
- **검사 게이트** `scripts/check-pickup-dayprior-reminder.mjs`: Code.gs 전체를 로드해 **진짜 발송 함수** 를 가짜 셀렉 시트·가짜 캘린더 위에서 돌리고 메일은 가로챈다(실제 발송 0). 대상·보류·본문 3개국어·요일·추가금·인화 전 문구·멱등·일정 변경·쿼터·상한·재조회·캘린더 실패·dryRun 문자열 — 조건을 하나씩 부순 **11종 전부 탐지**.
- 🔧 `check-widerruf` 가 "조기이행 열이 **맨 끝**이어야 한다" 로 적혀 있어 새 열을 붙이자 거짓으로 빨개졌다 → 진짜 의도(기존 열 위치 불변)대로 **인덱스 58 고정**. 새 게이트도 같은 방식(인덱스 59).
- 라이브: dryRun 정상(내일 9/27 일요일 → 대상 0), 셔틀 diag·견적 정상, 보드 응답 정상. 첫 실제 발송은 누군가 픽업을 예약하면 그 전날 08:00. 같은 시간대에 다른 세션이 @993(MRT 정산 지급일 액션)을 배포했다 — 배포 직전 라이브 대조에서 다른 곳은 내 구간뿐임을 확인했고 @994 가 그 코드를 포함한다.

### 2026-09-26 · MRT 정산 지급일 → 잔금입금일 정정 액션 `booking-set-mrt-payout` (메인 @993 · 게이트 28개째)

회계 세션이 §20 UStG Ist-Versteuerung(사장님 결정 2026-09-25)대로 마이리얼트립 매출을 **정산금 수취일**로 귀속시킬 수 있게 한 전용 쓰기 경로. 회계장부 `buildAccountingLedger_` 는 예약행 계상일을 촬영일(`row[0]`)로 잡되 `잔금입금일`(row[20])이 있으면 그 날짜로 덮어쓰는데, MRT 동기화(`bookingRow[BOOKING_COL['잔금입금일']] = fullPaidBySource ? today : ''`)는 그 칸을 **선결제 통지일**로 채운다. 실제 수취는 MRT 정산금이 신한계좌에 지급된 날(여행월 다음달 15일경 — 6월분 07-15 · 7월분 08-18). 그 날짜를 넣을 액션이 없었다(`booking-update` 허용 필드 밖 · `booking-confirm-balance` 는 Y 행 거부 · `booking-enrich-mrt` 는 연락처·인원·원화만).

- ✅ **`booking-set-mrt-payout`** — payload `rowIndex | rowIndexes[]`(같은 지급일 여러 건) · `expectName`(단건 전용) · `payoutDate` YYYY-MM-DD · `memo` · `dryRun`. `isMyRealTripBookingRow_` 통과 행만 · **잔금입금일 한 칸만** 교체 · 요청사항에 `[MRT 정산 YYYY-MM-DD] 잔금입금일 A → B (정산금 지급일 = §20 UStG 수취시점) · memo` 한 줄 append · 캘린더·메일 없음. **장부 로직은 손대지 않았다**(잔금입금일 오버라이드 그대로).
- 가드: 미래 날짜 거부(지급 확인 후 기록) · `2026-02-30` 같은 넘침 날짜 거부(정오 UTC 왕복 비교) · 취소 예약 거부 · 일괄은 **전부 검증한 뒤 기록**(하나라도 막히면 아무 행도 안 바뀜) · 같은 날짜면 `unchanged`(타임아웃 재시도 안전) · 응답에 행별 `previous`·`balancePaid`(잔금결제여부 Y 여부)·`shootDate`.
- 🧪 `scripts/check-mrt-payout.mjs`(신규 — `check-all` 이 자동 포함, 28/28 초록): 원본 함수를 떼어내 가짜 시트에서 단건·Date 셀·가드 9종·일괄 all-or-nothing·dryRun 검증. 결함주입 3종(MRT 가드 제거 · 미래 가드 제거 · 검증 중 즉시 쓰기)이 전부 빨개지는 것 확인. 사용 예는 `scripts/erp-agent.mjs` 헤더.
- 배포: 로컬 `appscript/` 가 스크립트 HEAD 와 6파일 전부 동일함을 `clasp pull`(격리 폴더) 대조로 확인한 뒤 push — AdminV2.html 의도치 않은 변경 없음. `--online` 게이트 28/28 → `-i` 재배포 @993, 버전 슬롯 160/200.
- 📌 **적용 진행(2026-09-26 10:30, 사장님 지시)**: 187 유성현 06-18→**07-15** · 205 김제윤 (없음)→**08-18** · 215 쪼울 07-19→**08-18** 반영 완료. 남은 것: 214·219·157 → 09-15 는 **정산금 입금 확인 후**, 9월 여행 5건(238·256·264·285·292)·296 은 **10월 지급일 확정 후**. 배포 직후 12건 `dryRun` 실측 — 전부 MRT 판별 통과:

  | row | 고객 | 촬영일 | 상태 | 잔금결제여부 Y | 현재 잔금입금일 | 넣을 지급일 |
  |---|---|---|---|---|---|---|
  | 187 | 유성현 | 06-17 | 작업완료 | ✗ | 06-18 | 07-15 |
  | 205 | 김제윤 | 07-06 | 작업완료 | ✗ | (없음 → 장부는 촬영일) | 08-18 |
  | 215 | 쪼울 | 07-30 | 촬영완료 | Y | 07-19 | 08-18 |
  | 214 | 진희수 | 08-12 | 촬영완료 | Y | 07-19 | 09-15 (지급 확인 후) |
  | 219 | 강예슬 | 08-15 | 작업완료 | Y | 07-21 | 09-15 (지급 확인 후) |
  | 157 | 김수정 | 08-30 | 촬영완료 | ✗ | (없음 → 장부는 촬영일) | 09-15 (지급 확인 후) |
  | 238·256·264·285·292 | 유소연·한혜정·연예진·김미빈·이가윤 | 09-02~09-23 | 촬영/셀렉완료 | Y | 08-07·08-24·09-01·09-15·09-22 | 10월 지급일 |
  | 296 | 김민정 | 09-25 | 촬영완료 | Y | 09-24 | 10월 지급일 |

  ⚠️ 187·205·157 은 잔금결제여부가 비어 있다(액션은 막지 않고 `balancePaid:false` 로만 알린다) — 회계 세션이 이유를 보고 정할 것.

### 2026-09-25 (저녁) · 여권 '기타' 국가도 한 나라로 — 2번째 국가면 €5 (메인 @992 · 셔틀 @22 · 예약 화면)

사장님 결정("받는 쪽"). 목록 밖 국가(🌐 기타 / 베트남 등 → 코드 `OTHER`)를 국가 수에서 빼고 세던 규칙을 없앴다 → **한국+기타 €30 → €35**, 기타 한 나라만은 그대로 €30, 5명 한국+기타 €157.50(가족할인). 예약 화면 안내("추가 국가는 1개당 €5", 3개국어)엔 원래 예외가 없었다 — 엔진만 안내보다 덜 받고 있었고 창구 '국가 추가'는 €5 를 받아 **경로마다 금액이 갈렸다**. 안내 문구는 그대로 둠.

- 서버 `calculateQuote_`·예약 화면 미리보기 `getPreviewQuote`·인보이스 품목 문구 세 곳을 같이 바꿨다(하나만 바꾸면 "화면 €30 / 청구 €35"). 엔진의 숫자 `*5` 는 `PASS_EXTRA_COUNTRY_FEE_` 로 — 보드 '국가 추가'와 같은 상수.
- 🔴 **게이트가 엔진을 안 보고 있었다**: 오전에 고친 `check-pass-country-memo` 는 상수를 소스에서 읽었지만 엔진은 그 상수가 아니라 숫자 5 를 직접 쓰고 있었다. 이제 **Code.gs 전체를 GAS 스텁과 함께 로드해 진짜 `calculateQuote_`·`buildInvoicePricingPreview_`·`syncBookingFromInvoiceRecord_` 를 실행**한다(공식 복제 없음). 결함주입 11종 전부 탐지(엔진·화면·상수·인보이스 문구·어드민 계산기·발행 동기화).
- ⚠️ **셔틀 생성기가 내 실수를 막았다**: 엔진 주석에 쓰기 함수 이름을 적었더니 추출기(주석도 훑음)가 그 함수와 의존 15개를 조회 전용 셔틀로 끌어왔고, 쓰기 호출 스캔이 `❌ .setValue(` 로 거부했다. 주석에서 이름 제거 → 셔틀 변경분은 엔진 한 줄 + 상수 하나(함수 178개 그대로).
- 배포 전 적대적 검토(3차원×회의론자) 확정 6건:
  - 🔴 **인보이스 발행 계산기가 국가 수를 늘 1 로 보냈다**(기타만이 아니라 한국+독일도) — 예약을 불러오면 자동 계산이 돌아 €35 예약에 €30 을 띄우고, '장부 자동 반영'이 켜져 있으면 총액까지 €30 으로 덮어쓸 수 있었다. 예약을 불러오면 칸을 비우고(= 예약 기준) `rowIndex` 를 보내 서버가 그 예약의 구성을 읽는다. 발행 페이로드의 `||1` 되돌림도 제거. 라이브 `invoice-preview`: 한국+기타 €35 '1인당 국가 2개' · 창구 등록 3명 €105 · 직접 입력 1 은 €30(우선).
  - 🔴 **발행 동기화가 옵션 열을 통째로 바꿔 창구 등록의 `국가 N개/인` 기록을 지웠다**(9/23 상품·인원 경로에서 막은 것과 같은 소실) → 보존. 진짜 함수를 가짜 시트 위에서 실행해 검증(라이브 발행은 STMIN 연번 소모라 하지 않음).
  - 입력칸 예시 "예: France, Canada" → "예: France"(3개국어) — 한 칸에 여러 나라를 적어도 한 나라로만 청구된다.
  - 셔틀 동시 배포: 셔틀이 옛 엔진이면 예약 화면 확인 단계가 €30 을 보여주고 서버는 €35 를 청구한다. 배포 순서를 **셔틀 → 메인 → 화면**으로(전환 중에도 화면 금액 ≥ 청구액). 라이브 두 서버 모두 5개 경우 일치 확인.
  - 기존 예약 영향 조사: 2026 여권 예약 183건 중 목록 밖 국가가 2번째 국가인 건 **1건(행 147, 5월 작업완료)** — 앞으로 잡힌 예약은 0건. 코드 보정 없음.
  - 🇰🇷 기본 선택 때문에 외국인 고객이 한국을 끄지 않으면 €5 가 붙는 함정 — 데이터상 실제로 걸린 예약 0건(독일어 화면 3건 전부 한국 없이 접수, 한국어 화면도 52건이 한국을 끄고 접수). 주력 흐름이라 손대지 않음.
- 📌 별도 문제로 남김: 보드 '상품·인원' 변경은 예약 전체를 새로 견적하므로 잔금 완료 건에서도 총액을 덮어쓸 수 있다(결제 가드 없음, 이번 변경과 무관한 기존 동작).

### 2026-09-25 · 검사 게이트 단일 러너 + 결함주입으로 드러난 커버리지 구멍 (게이트만 변경 · GAS 배포 없음)

9/23 다른 세션의 점검이 남긴 미결 — **"아무도 게이트 전부를 돌리지 않는다(단일 러너 없음)"** — 을 닫고, 게이트가 정말 뭘 막는지 실측했다.

- ✅ **`node scripts/check-all.mjs`** (신규): `scripts/check-*.mjs` 26개 + `verify-release-gate.mjs --offline` 를 동시 실행해 한 표로 보여주고 하나라도 실패하면 exit 1. 전부 오프라인·읽기 전용이라 **0.6초**다 — 아무도 안 돌린 이유가 비용은 아니었다. `--online` · `--only <문자열>` · `--quiet` · **`--future N`**(프리로드 `scripts/lib/shift-clock.mjs` 로 시계를 N일 앞으로 밀어 실행 — 고정 날짜 픽스처가 상대 창에서 미끄러지는 부패를 미리 잡는다). 배포 문서 두 곳의 첫 명령을 이걸로 교체.
  - 러너가 **실제로 빨개지는지 실증**: 실패·throw 게이트를 심어 exit 1·본문 출력·표 요약을 확인(못 빨개지는 러너는 없는 것보다 나쁘다). throw 한 게이트의 마지막 줄이 `Node.js v24…` 배너라 요약이 쓸모없던 것도 수리.
  - 현재 상태: **27/27 초록**, +120일·+400일 시계에서도 초록.
- 🔬 **결함주입 실험**(격리 사본 `/tmp` 에서만 — 라이브 `appscript/Code.gs` 는 다른 세션이 push 할 수 있어 절대 건드리지 않았다): Code.gs 상수를 하나씩 틀고 27개를 돌려 누가 잡는지 표로 만들었다. 최상위 숫자 상수 24개 중 **게이트가 지키던 건 4개**. 나머지는 대부분 캐시 TTL·스캔 상한 같은 운영 노브(고정할 가치 없음)였지만 아래는 진짜 구멍이었다.
- 🔴 **부가세 19% 가 어디에서도 게이트되지 않았다** → 수리(`verify-release-gate.mjs`): `CONFIG.QUOTE_VAT_RATE` 를 0.19→0.07 로, 장부·환불·세액 환산의 `/1.19` 를 `/1.07` 로 바꿔도 **27개 전부 초록**이었다. ELSTER 에 신고하는 넷토·세액이 통째로 틀어지는 숫자다. 이제 ① 설정 상수 고정 ② **주석을 걷어낸 코드에 7%·16% 환산(1.00~1.18)이 끼어들면 빨강**(레이아웃용 1.45·1.7 은 주석이라 오탐 없음) — 돌연변이 5종 전부 탐지 확인.
- 🔴 **게이트 없던 돈·법정 숫자 3개** → 고정: `LOYALTY_CREDIT_EUR_=20`(3회차 혜택, 고객 청구액) · `CONTRACT_MIN_TOTAL=500`(이 금액부터 계약서 의무) · `TRAVEL_KM_RATE_=0.30`(EStG 자가용 km 공제, 세무 숫자) · `GUTSCHEIN_VALID_MONTHS=36`(발행연도+3년 안내와 일치).
- 🔴 **내가 9/23 만든 게이트가 금액을 검증하지 않고 있었다** → 수리: `check-pass-country-memo.mjs` 가 `PASS_EXTRA_COUNTRY_FEE_`·가족할인율·최소인원을 **하네스에 하드코딩**해, Code.gs 에서 5→4 로 바꿔도 초록이었다(헤더는 "금액 전부 일치" 라고 주장). 이제 세 상수를 Code.gs 에서 읽고, 기대값은 사업 사실(140€·157.50€…)로 독립 고정한다 — 순환 단정 2개도 리터럴로 교체. 돌연변이 2종 탐지 확인.
- ✅ **9/23 세션이 남긴 하네스 수리 6건 커밋**(그 세션은 종료됨): 추출 누락 보강(`selectPrintCreditExempt_`·`formatDateMinute_`·`roundCurrency_` 등), 인화 볼륨 사다리를 @945 라이브값(5:10/10:15/20:20)으로 정정, 3개국어 throw·리팩터로 어긋난 구조검증 정규식 갱신, 메일 순서 의존(`sent[0]`/`sent[1]`)을 역할 조회로 교체. 전수 검토 결과 **약화된 단정 없음**(오히려 스킵 케이스 1건이 실제 단정으로 승격, 복구 이벤트 제목 검사가 엄격해짐) — 라이브 Code.gs 와 기대값 재확인 후 커밋.
- 🔍 **적대적 감사(3차원 × 회의론자) 확정 6건 전부 수리** — 초록이던 게이트들이 실제로는 안 보던 것들:
  - 🔴 **크리티컬**: `check-select-free-cut-prints` 가 인화 단가를 **하네스 안 사본**으로 돌리고 있었다(`premium_a3` 50/35 vs 라이브 38/32). "서버 엔진을 실제로 실행한다"면서 **단가는 한 개도 라이브와 대조되지 않았고**, A4→A3 차액 단정이 존재하지 않는 €25 를 고정하고 있었다(실제 €22). 원본 `PRINT_LABELS` 추출로 교체 + 기대값 정정 → 이제 단가를 틀면 **3개 게이트**가 잡는다(과거 실제 사고였던 `basic_a4` 보정본가 0 드리프트 포함 실증).
  - 🔴 **은퇴한 규칙을 적은 주석 때문에 통과하던 단정**: `verify-release-gate [A4]` 의 '암트'·'돌잔치'·'가족파티' 검사는 €20 패턴이 아니라 **"이들은 €10"이라고 적어 둔 주석**에 걸려 통과했다. 그 빨강을 "고치면" 2026-09-17 에 은퇴한 결함(안내 €20 ↔ 청구 €10)이 되살아난다. 주석 제거 후 **있어야 할 것(웨딩)은 있는지 + 은퇴한 셋은 없는지** 양방향으로 본다.
  - 🔴 **내가 만든 러너의 구멍**: `check-all` 이 기본 `--offline` 이고 릴리스 게이트가 **건너뜀을 '통과'로 계산**해, `[Phase 1] 내부 단가 노출`·`select 루트 → /v2/ 301` 이 배포 흐름에서 **한 번도 돌지 않았다**. 건너뜀을 경고로 분리, 요약에 명시, 배포 문서를 `--online` 으로 교체 → 라이브 29건 전부 통과 확인.
  - 추출 랜드마인 2개(둘 다 Code.gs 의 try/catch 가 ReferenceError 를 삼키고 있었다): `invalidateTodayBoardCache_`(수납 경로 매 호출) 는 스파이 스텁으로, `getDbSheet`(닿지 않는 폴백) 는 시끄럽게 실패하는 스텁으로.
  - `extractFn` 사본 2곳(`check-extra-days`·`check-pay-method`)은 정규식 리터럴을 몰라 짝 없는 인용부호를 만나면 **뒤따르는 함수 수백 개(최대 413KB)를 조용히 함께 삼킬 수 있었다** → 추출 결과에 열 0 `function` 이 있으면 throw 하는 한 줄 가드(오탐 0 확인).
  - `check-month-close-record` 가 Code.gs 의 독일식 금액 파서 대신 3줄 스텁을 쓰고 있었다 — `'5.566,00'` 을 **0** 으로 읽는다(시트가 문자열로 돌려주는 실제 형식). 원본 `parseMoneyValue_` 로 교체 + 독일식/영미식/통화기호 왕복 단정 6개 추가(42→48).
- 📌 미결로 남긴 것: 여권 가족할인 **상한**(`parsePercentSetting_(v,10,50)`) 은 여전히 미탐지(시트에 사장님이 직접 입력해야 발동하는 상한이라 보류) · 하네스 16곳의 `extractFn` 복붙 통합(리팩터 규모가 커서 보류 — 이제 추출 누락은 `check-all` 이 다음 실행에서 즉시 빨갛게 만든다).

### 2026-09-23 · 여권 인원 추가 기능 + 그 과정에서 드러난 국가 요금 누락 4곳 (메인 @988~@991 · 오늘촬영.app 재빌드)

사장님 요청("김여주 인원 추가 2개 국가 여권까지, 오늘촬영 앱에 인원 추가가 없어")에서 시작해 **기능은 이미 있었고 금액이 틀리던** 문제로 번졌다.
여권가는 사람마다 `30€ + (국가수−1)×5€`, 5인부터 가족할인 10%. 1인당 국가 구성은 시트 열이 아니라 **요청사항 메모의 `[국가별 신청] 4명:한국+독일` 토큰**(온라인 예약 프런트가 씀) 또는 **옵션 열의 `국가 2개/인`**(창구 수기등록이 씀)에만 있다. 재견적 경로들이 이걸 안 읽어 **덜 받고 있었다.**

- 🔧 **보드 '상품·인원' (@988)**: `changeBookingProductForAgent_` 가 `calculateQuote_` 에 국가 구성을 안 넘겨 4명 한국+독일이 **140€ → 120€** 로 계산됐다(그 버튼을 오늘 쓰셨으면 €20 손실). `_expandPassPersonCountries_` 로 메모 토큰을 1인당 배열로 펼쳐 넘기고, 저장 시 토큰을 새 인원으로 다시 쓴다. 검증: 4명 140 · 5명 157.50(가족할인 17.50) · 6명 189 · 비상속 5명 153.
- 🔧 **창구 수기등록 건 (@990)**: 메모 토큰이 없고 국가가 옵션 열 `국가 2개/인` 에만 있는 행은 인원을 올리면 여전히 전원 기본가였다(2명 70€ → 3명 **90€**, 정답 105€). 게다가 저장이 옵션 열을 옵션키로 덮어써 **국가 수 기록 자체가 사라졌다**(복구 불가). `resolvePassPersonCountriesForRow_`(메모 토큰 1순위 → 옵션 국가수 폴백) 한 곳으로 모으고, 옵션 쓰기는 `국가 N개/인` 표기를 보존한다. 라이브 임시행으로 2명 70 → 3명 **105** → 쓰기 후 4명 **140**(옵션 보존 확인) → 행 삭제.
- 🔧 **어드민 수정 모달 (@990)**: `buildInvoicePricingPreview_` 는 국가 수를 못 받으면 1개로 가정하고 법인 여부도 안 넘겼다 → 4명 한국+독일을 5명으로 올리면 **135€**(정답 157.50€)가 금액칸에 찍히고, 법인 여권 건에도 가족할인이 붙었다. `rowIndex`(+`bookingRowIndex` 별칭)를 받아 예약행에서 구성을 읽고 `businessInvoiceNeeded` 를 전달. 인보이스→예약 동기화 미리보기도 같은 인자를 넘긴다. 라이브 `invoice-preview` 5명 = **157.50**.
- 🔧 **보드 '국가 추가' (@990)**: `fee×국가수×인원` 산술이 **가족할인을 몰랐다** — 5명 157.50€ 에 +일본이 +25€(182.50)를 붙여 엔진값 180€ 와 어긋났고, 다음 재견적에서 조용히 되돌아갔다. 차액을 엔진 전/후 견적차로 계산(총액은 덮어쓰지 않는다 — 수기 할인 행 보호). 라이브 임시행: 135€ → +일본 **+22.50€ = 157.50€**, 엔진 재견적과 일치.
- 🔧 **인보이스 품목 문구 (@991)**: 금액은 국가 2개로 계산하면서 설명에는 `1인당 국가 1개` 가 찍혔다(고객 문서). 실제 견적 구성에서 센다 — 사람마다 다르면 `국가 구성 2개/2개/1개`.
- 🔧 **표기 보존**: 목록에 없는 국가('베트남')는 코드가 OTHER 라 토큰을 다시 쓸 때 **'기타' 로 뭉개져** 어느 나라 규격으로 인화할지 근거가 사라졌다. 같은 구성이면 원문 표기를 유지한다.
- 📱 **오늘촬영.app**: 버튼 `상품 변경` → **`상품·인원`**, 여권에서 인원을 늘릴 때만 나오는 토글 **"추가 인원도 기존과 같은 국가 구성"**(기본 ON, 끄면 추가 인원 기본가), 미리보기에 가족할인 줄, 서버가 알려주는 현재 국가 구성 표시. 디바운스 350ms 동안 '변경' 버튼을 잠갔다 — 예전엔 금액을 본 뒤 인원을 한 번 더 눌러도 버튼이 살아 있어 **화면에 없던 금액으로 저장**될 수 있었다.
- ✅ **검사기 추가**: `scripts/check-pass-country-memo.mjs` — 파싱·펼치기·**왕복**(저장한 토큰을 다시 펼쳐도 금액 불변)·창구 폴백·표기 보존·법인 제외까지 25개 단정. 국가 관련 코드를 만지면 이걸 먼저 돌린다.
- ⚠️ **사장님 결정 1건**: 목록에 없는 국가(`기타`)의 2번째 국가 요금 — **창구 '국가 추가'는 €5를 받고, 견적 엔진은 안 받는다**(온라인 예약에서 '기타 국가'를 2번째로 고른 손님은 지금 추가금 없이 지나간다). 작업량은 같으니 받는 게 맞다면 엔진을 고쳐야 하는데, 그건 **고객 화면 가격이 바뀌는 변경**이라 지시 없이 손대지 않았다.
- 김여주 row 294: 4명 140€ · 현금 완납 · 촬영완료(장부 9/23 매출 140€, 미수 0). 인원 정정 때 변경안내 메일이 1통 나간 건 이미 보고드렸다(`booking-update` 는 notify 를 전달하지 않음 — 정정 경로는 `booking-change-product`/`booking-set-amount` 를 쓸 것).
