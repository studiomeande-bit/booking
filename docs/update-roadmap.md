...
### 2026-09-23 · 프로필 다인 컷 = 인원 기준 보정 슬롯 (메인 @989 · 프런트 배포)

사장님 결정 2026-09-23: **프로필(prof)은 1인 기준 가격**(pb 1장 / pbus 2장 / pp 3장)이라, 한 컷에 여러 명이 나오면
포함 보정 슬롯을 **인원수만큼** 쓴다 — 1인=1 · 2인=2 · **3인 이상=3**. 초과분은 기존 추가보정 단가(€10/슬롯)로 청구하고
**새 단가를 만들지 않는다**. 돌 가족컷도 예외 없음(가족사진을 원하면 스튜디오 상품으로 안내).
스튜디오·스냅·웨딩·마이리얼트립은 **종전대로 장 단위**(인원 무관).

- **인원을 아는 유일한 경로는 고객 신고**다. 셀렉 보정 단계 각 컷에 「이 컷에 나온 인원」 1 / 2 / 3+ 세그먼트 컨트롤을 추가(기본값 1,
  프로필 세션에서만 노출). 카운터를 장수 대신 슬롯 기준으로 바꿔 `보정 3장 중 3장 사용 (2컷 선택)` + 추가금 미리보기를 그 자리에서 보여준다.
  유료 배지는 **그 컷에서 유료로 넘어간 슬롯 수만큼** 금액을 곱한다(기본 1장 세션의 2인 컷 = 2슬롯 중 1슬롯만 유료).
- **서버가 청구 정본**(`computeSelectExtraRetouch_`): 클라이언트 합계를 믿지 않고 사진별 `persons` 를 1~3 으로 클램프해 다시 센다.
  `enrichSelectPhoto_` 가 저장 시 정규화하므로 시트/작업지시서/수정제출 경로가 모두 같은 값을 본다.
- 무료 슬롯 규칙 유지: 빈 보너스·서비스 행(placeholder)은 슬롯을 소비하지 않고, 남는 마케팅 보너스는 일반 초과분을 계속 흡수한다.
  다인 컷은 **잔여 슬롯이 그 컷의 인원수보다 적으면 통째로 유료 후보**로 내려간다(캡 초과 플래그 사진 처리와 동일 규칙).
- 관리자 제출·수정 알림과 고객 영수증의 보정 목록에 `2인 컷` / `3인 이상 컷` 배지(고객 메일은 3개국어), 어드민 셀렉 상세(작업지시)의
  사진번호 옆에도 같은 배지 — 리터처가 "이 컷은 3인 보정" 을 바로 본다. 알림의 '보정선택' 칸은 `3컷 · 보정 5장분(인원 기준 · 기본 3장)` 로 표기.
  고객 영수증엔 컷수와 보정 장수가 왜 다른지 한 줄 설명을 붙였다(3개국어).
- **보정 범위 안내를 `prof`·`stud` 로 확대** — 9/23 프로필 고객의 얼굴 합성 요청의 원인이 "이 안내가 snap·마이리얼트립에만 떴다" 였다.
  단 제외 목록은 **사장님이 확정한 3가지(합성 · 사람 제거 · 체형 합성)만** 쓰는 축약본(`scopeOutCore`)을 쓴다 — 스냅용 목록의
  '의상 주름 제거'까지 얹으면 스튜디오·프로필이 약속하는 범위를 임의로 좁히게 된다. 인라인 경고 정규식도 같은 이유로 '주름' 을 뺀
  좁은 쪽(`RETOUCH_SCOPE_CORE_RE`)을 쓴다. 관리자 알림의 ⚠️ 배지만 종전 넓은 정규식 그대로(내부용이라 과검출 비용이 싸다).
- 검증: `scripts/check-retouch-slots.mjs` 신설 — Code.gs 원본 함수를 떼어내 슬롯 계산 34건(정규화·상품별·타 촬영군·보너스/서비스컷 상호작용)을
  대조하고, **select.js 미러와 서버가 같은 답을 내는지** 9건을 교차 확인한다. 기존 `check-select-amounts`(4019건)·`check-select-included-prints`·
  `check-print-prices`·`verify-release-gate` 통과.
- **예약 276행(오세호)은 손대지 않았다** — 규칙 이전 건으로 추가금 없이 종결된 예외.

- **라이브 검증(테스트 세션 전용, 실제 고객 세션 미사용)**: 수신 메일이 사장님 주소로만 가도록 테스트 셀렉 세션 2건을 만들어 확인했다.
  - 서버: `1인 프로필 Basic`(기본 1장) 세션에 **3인 컷 1장** 제출 → `extraRetouch 2 · €20`. 규칙대로 1슬롯 포함 + 2슬롯 유료.
  - 관리자 알림: `보정선택: 1컷 · 보정 3장분(인원 기준 · 기본 1장) (추가 2장 × 10€ = 20€)`, 작업지시 줄 `0012번 [3인 이상 컷]`.
  - 고객 영수증: `보정 선택: 1장 (+2장 × 10€ = 20€)` + `프로필 촬영은 1인 기준 가격이라 … 1컷 = 보정 3장분` 안내.
  - 화면(라이브): 카운터 `보정 1장 중 3장 사용 (3컷 선택)` · 추가금 `추가 2장 × €10 = €20` · 해당 컷 배지 `+€20` — 서버와 일치.
  - 프로필 세션에 보정범위 안내가 뜨는 것, 제외 목록이 축약본 3항목인 것, '옷 주름 펴기' 예시는 좋은 예시로 **그대로 남는 것**까지 확인.
  - 뒷정리: 셀렉 2행 + 인화장부 테스트 행(€20 미결제) 삭제, 잔여 0건. 예약 276행(오세호)은 추가금 행 자체가 없어 무변동 확인.
  - 부수: `?preview=1&group=prof|stud|snap|wed` 로 미리보기 촬영군을 바꿀 수 있게 했다(고객 세션 없이 촬영군별 UI 확인용).

⚠️ 미결: `scripts/check-select-free-cut-prints.mjs` 는 **이번 작업과 무관하게 이전부터 깨져 있다**(`selectPrintCreditExempt_` 가
추출 목록에 없어 ReferenceError). HEAD 에서도 동일 — 별도 수리 필요.

### 2026-09-22 · 셀렉 페이지 정상작동 점검 (메인 @987)

테스트 세션(수기등록 → 고객 메일 없음)으로 고객 화면부터 제출까지 확인 후 전량 삭제(예약·셀렉·인화장부 행 0 잔여).

- **읽기**: 페이지 로드 → 세션·상품·기본보정 3장·단가 €10 정상 표시, 콘솔 오류 0. 조회 2종 모두 **셔틀**(session 2.6s · photos 8.5s 콜드/신규 세션). 갤러리 169장 로드, 별점·필터(⭐3 · 5점 3)·1→2단계 이동·가드 문구("보정할 사진을 고르지 않았습니다…") 정상.
- **쓰기(어제 바꾼 가격 권한 검증)**: `select-submit`·`select-update` 에 **가격 0 + 라벨 위조**(`price:0`, `label:'무료라고 우기기'`)를 실어 보냄 → 서버가 전부 덮음. 저장값 `시그니처 10×15cm ×17 (3€)` = 카탈로그 라벨·보정본 단가, 20장 중 **3장은 상품 포함분이라 무과금**, 볼륨할인 15% 적용 → **43.35€**. 재방문 시 '기존 제출 내용을 불러왔습니다' 로 복원.
- 🔧 **부작용 1건 수리(@987)**: 어제 넣은 사진번호 정리 `slice(0,24)` 가 **파일명을 잘랐다** — 이 갤러리는 번호 대신 파일명이 들어온다(피커가 파일명을 넣음). `251004_Jenna Knight2513_Original` → `251004_Jenna Knight2513_` 로 작업지시서·인보이스 품목에 남아 파일 특정이 어려웠다. 80자로 완화하고 재제출로 전체 파일명 복원 확인.
- 참고: UI 자동 클릭으로 피커 썸네일을 고르는 건 실패(합성 click 무시·좌표 클릭이 미리보기 모달을 염) — 쓰기 경로는 페이지가 쓰는 것과 **같은 payload** 로 API 직접 호출해 검증했다.
...
- **뿌리 원인 — google.script.run 표면**: 웹앱이 ANYONE_ANONYMOUS + 소유자 권한이고 doGet 이 누구에게나 HtmlService(어드민 로그인 화면)를 내주므로, 이름이 `_` 로 **끝나지 않는** 최상위 함수는 익명 호출이 된다(앞의 `_` 는 비공개가 아니다). 열려 있던 것: 메일 헬퍼(`_sendConfirmEmail` 등 — 스튜디오 Gmail 명의로 임의 수신자·임의 HTML·캡 없음), 트리거 핸들러(아침 리포트·SumUp·MRT 동기화·셀렉 자동 점검·dailyTasks·인스타 게시 — 일부는 고객명·캘린더 eventId 반환), 설치 함수, `processForm`(라우트의 허니팟·requestId 우회).
  수리: `isTrustedInvocation_(e)`(설치된 triggerUid 대조 **또는** 활성 사용자=실행 사용자) 가드 + 오작동 시 6시간 1회 사장님 알림 메일, 편집기 실행이 필요한 함수는 껍데기+`*_` 본체 분리, 메일 헬퍼 9개·`processForm` 을 `*_` 로 개명. 스윕: 최상위 1,407개 중 호출 가능 239개 → 무인증 46개를 전수 분류.
- 🔒 **어드민 저장형 XSS(치명 1 · 높음 4)**: 고객 입력이 escape 없이 innerHTML 로 — 일정 변경 사유(접수 메일만 받아도 서명 링크가 생긴다), 셀렉 사진 메모·번호·인화 라벨, 인화 이력, **예약 폼의 VAT번호·참조번호**(인보이스 모달·목록·검증 모달·인쇄창), 인보이스 품목명, 인화 탭 고객 검색. onclick 속성에 `JSON.stringify(inv)`/고객명을 넣던 자리는 `&quot;` 한 줄로 핸들러가 깨져 인덱스 조회(`allInvoices[i]`)로 교체. 서버도 출처에서 정리(`<>` 제거·길이 제한).
- **가격 권한**: `passAddonPeople` 음수로 총액·계약금 인하 → 1~10 클램프. 여권은 국가 구성을 덜 보낸 인원도 기본가로. 셀렉 인화는 **항상 분리형 + 서버 카탈로그 가격**(레거시 경로는 v1 페이지가 없어 공격자만 고를 수 있었고 클라이언트 가격 0·음수를 합산했다), 라벨은 서버 것·사진 번호는 안전 문자만.
- **입력·릴레이**: 예약 메일의 passCountries/otherCountry HTML 주입, 대기 등록(이름·상품 원문 + 쉼표 다중 수신자 + requestId/허니팟 없음), 시트 수식 주입 싱크 3곳(`neutralizeRow_`: 메시지 로그·협력업체 클릭·인화 시트), 일정 변경 제출(상태 게이트·'YYYY-MM-DD HH:mm'·고객에게 보여 준 슬롯 안의 시각만·접수 후 2분 쿨다운·재요청 시 '기존' 보존).
- **최소 공개·상한**: 공개 `contact-lookup` 은 이름·방문횟수만(전엔 이메일+전화를 아는 제3자가 집 주소·다음 예약일을 받았다 — 어드민·에이전트 경로는 전체 필드 유지), 주소 조회 프록시 5분 40회·200자, 공개 메일 캡을 **용도별 버킷**(`meta.bucket`: print·wdr·select)으로 나눠 다른 폼 홍수가 철회·출력주문 알림을 굶기지 않게, 셀렉 제출·수정·출력주문·미연결 철회 알림을 캡 아래로, 상담 폼의 자동 캘린더 이벤트는 08~20시·지평선 안·시간당 5건(그 밖은 '신규'+희망일정으로 접수만), 원시 달력 조회(`getUnavailableDays`·`getAvailableSlots`)에도 창 검사.
- **④ 트리거 런타임**: 메인 5분 워밍에서 월 이벤트 재계산 제거(TTL 120초 < 5분이라 매 실행 콜드, 그 캐시를 읽는 공개 표시 경로는 셔틀이 서빙) — 추정 50~100분/일 절감, 웹앱 핑만 유지. 운영 로그 12일·200행 실패 0건(한도 초과 증거는 없었음, 예방 조치).
- **생성기 함정**: 추출기는 주석도 훑는다 — 셔틀 폐쇄 안 함수 주석에 `dailyTasks` 를 적었더니 Public.gs 가 179 → 740 함수·쓰기 호출 포함으로 부풀었고 **쓰기 가드가 잡았다**. `warmupCacheTrigger` 를 exclude 에 추가.
- **라이브 검증**: 음수 인원 클램프(200=200) · 여권 3인/구성 1개 = 3인가(90) · 필드 정리 · contact-lookup 최소 응답 · waitlist requestId 요구 · slots-month 31일 · 릴리스 게이트 23 통과 · **공개 예약 종단 1건**(사장님 메일·2027-05-04 09:30, `processForm_` 경로 → 행 291 생성 → 즉시 삭제·캘린더 삭제·슬롯 복귀 확인) · 가드 오작동 알림 메일 0건(배포 후 5분 트리거 5회 경과).
- 보류: eventId 를 그대로 받는 예약 조회 ref(낮음 — MRT 트리거 유출 경로는 막힘, 옛 메일 링크 호환 확인 후), 철회 응답의 일치 여부 노출(낮음), `ACTION_SECRET` 하드코딩 폴백 fail-closed(board-api 영향 확인 후).
...
- **포트폴리오 협업(TFP) 전용 랜딩** `booking.studio-mean.com/portfolio/` — 상호 무페이·60분·사용권 동의 필수, 3개국어, noindex.
  상품 `tfp_portfolio`(g/t=`tfp`, €0, 60분+준비 15분)는 상품설정 시트 밖 하드코딩(프로모 상품과 같은 패턴).
  메모에 `[포트폴리오 협업 · TFP] 상호 무페이 · 사용권 동의함` 토큰을 박아 장부·보드에서 식별된다.
  - **@934**: `sanitizeInitDataForApi_` 화이트리스트에 `tfpProducts` 추가 — 없으면 공개 init 에서 잘려 페이지가 영구 잠김.
  - **프런트 버그 2건** (라이브에서 달력이 전부 열려 보이고 시간대가 한 칸도 안 나오던 원인):
    `calendar-batch` 의 `month` 는 JS `getMonth()` 와 같은 **0-based** 이고 응답이 `` `${year}_${m0}` `` 로 한 겹 감싸여 있으며
    키는 `closedDates` 가 아니라 `closed`/`unavail` 이다(promo.js 가 정본 패턴). / `fetchSlots` 는 **배열을 그대로** 반환한다.
  - 라이브 확인: 9월 휴무·만실 반영, 9/16 슬롯 10개, 견적 €0·75분, 서버 수락.
- **인화·액자 단가** `docs/print-frame-pricing-2026-09.md` 2차 개정 — 독일/유럽 랩을 3등급(프리미엄 파트너·프리미엄 랩·소비자)으로
  넓혀 **실지불 기준**(최소주문·배송 포함) 재실측. A4 €15 는 이미 시장 최저권이라 유지, A3 는 하향(38/32), 액자 추가금도
  +29/+35 로 하향(프리미엄 DIY 합계 €59 대비 +14%). **사장님 결정 대기 4건.**
- **워크인 → 예약행 이메일 유실** (나용민 행273): 수기 생성 시 워크인 시트의 실제 메일 대신 `수기등록` 이 들어가
  여권 사진 전달이 막혔다. 행273 정정(`booking-update` notify:false) + 앱 대화상자가 발송 불가 상태에서
  "확인을 누르면 메일이 나갑니다" 를 띄우던 모순 제거 + **@935** 서버 에러 문구에 조치 안내 추가.
  어드민 `prefillManualFromWalkin` 은 이미 이메일을 옮기고 있다 — 에이전트로 직접 만들 때만 새는 경로.


### 워크인 현장 처리 3종 + ERP 모바일 레이아웃 (2026-09-09 저녁, @930–@933 · board-api @8 · 앱 재빌드)

- **계기**: 워크인 나용민 €60 현금을 받았는데 오늘촬영에 안 뜬다는 사장님 신고. 원인 = 워크인은
  '워크인접수' 시트에만 쌓이고 예약장부 전환이 **수동**이라 보드·매출·현금장부 어디에도 없었다.
  즉시 처리(예약행 273 · 현금 60 수납 · 워크인 '예약등록' 마감) 후 구조를 고쳤다.
- **보드 워크인 큐**(@930): `readTodayWalkins_` — 오늘 접수 + 미전환 건을 붉은 카드로 + 상단 경고문.
  실측 검증(상태를 되돌려 1건 표시 확인 후 복구). 접수일시가 Date 객체여도 parseDateSafe_ 로 정규화.
- **앱 워크인 등록**(@932, `walkin-quick-add`): 보드 하단 「+ 워크인 등록」 → 이름·연락처·상품(라이브
  카탈로그)·인원·금액·결제수단 → **한 왕복에 예약행 생성 + 수납 기록 + 촬영완료**. 가격은 서버가
  상품설정 시트에서 재확인(앱 캐시 불신). 겹침 허용(워크인 특성), 고객 메일 없음, 미결제면 미수로 남김.
- **일찍 시작 반영**(@931): 시작 표시가 늦은 경우만 지연에 반영돼 앞당김이 무시됐다 → 음수도 기록해
  종료예정·다음촬영 카운트다운이 실제와 맞음(±240/480분 클램프).
- **인보이스 발행 버튼**(앱): 촬영 카드에서 `invoice-create` — "STMIN 연번 즉시 소모" 확인창, 발행 후
  PDF 자동 열기. 고객 발송은 어드민에서 별도.
- **워크인 이메일·요청사항 노출**(@933): 시트엔 있는데 `walkin-list`·보드 큐 응답에서 빠져 있었다.
- **ERP 모바일**(@933): ① 메뉴 19개가 폰에서 한 줄 가로 스크롤이라 3개만 보였다 → 칩 줄바꿈(그룹별
  구분선)으로 전부 노출 + sticky 해제(세로 차지 완화). ② 표 `min-width:1100px` → 768px 이하 820,
  480px 이하 700 + `.table-wrap` 높이 제한 해제. ③ side-nav sticky 오프셋을 축소된 top-bar 높이에 맞춤.
  검증: AdminV2 스타일·내비를 그대로 뽑아 375×812 로 렌더해 확인(칩 5줄 · 표 가로 스크롤).

### 오늘촬영 앱 — 자정 넘기면 자동으로 오늘 날짜로 (2026-09-08, 앱 재빌드)

- 사장님 요청: "하루에 한 번 자동으로 오늘 날짜로 전환". 앱을 켜 둔 채 하루가 바뀌면 어제 보드에 머물러
  아침에 빈 화면을 봤다.
- 1초 타이머(시계·알림 평가)에 달력 날짜 감시 추가: `lastDayKey` 가 바뀌는 **그 순간에만** `viewDate=Date()`
  → didSet 이 로드+인접일 프리페치. 날짜가 안 바뀌면 아무것도 안 하므로, 사장님이 다른 날짜를 보고 있어도
  자정 전까지는 건드리지 않는다. 맥이 자다 깨도 타이머 재개 즉시 스냅.
- 날짜 키는 맥 로컬 시간대(=Europe/Berlin) 기준으로 서버와 동일.

### 8/29 마감 대조 추적 — 계좌이체 해소, 카드 +35 는 사장님 확인 대기 (2026-09-05, @928–@929)

- **계좌이체 130 해소**: 김정음 계약금이 은행에 **8/28** 착금(Jeongeum Kim 130 · 정산행 386, 예약행 259 매칭
  완료)인데 ERP 계약금입금일만 8/29 로 기록돼 있었다 → `booking-confirm-deposit`(paidDate 8/28, 계좌이체,
  이미 입금완료라 메일 없음)으로 실제 입금일에 맞춤. **8/28 마감 대조 완전 일치**(카드 65=65, 계좌 130=130).
- **카드 +35 미해소**: SumUp 8/29 5건=360 vs 장부 카드 325. 초과분은 `TAAA43E49ZZ`(카드사 **none**) —
  자동매칭이 최예준(row 253)에 붙였지만 그 예약의 결제수단은 **현금**. ⓐ 카드 결제였다면 결제수단 정정,
  ⓑ SumUp 에 기록된 현금/별건이면 정산행 처리 — **SumUp 앱에서 그 거래의 결제수단 확인** 또는 그날 서랍
  현금(ⓐ 310 / ⓑ 345)으로 판별. 사장님 확인 대기.
- 부수: day-close 버킷을 소스 대신 회계분류로 가르도록 보강(@928) — ⚠ 매칭 시 회계분류가 매출분류로
  덮어써져 **미매칭 SumUp 현금행에만** 걸린다(한계 주석 명시 @929).

### 마감 대조 — 삭제된 현금 수기 행이 '실제 현금'으로 되살아나던 버그 (2026-09-05, @927)

- 어제 마감 대조 시트가 9/2 현금을 "장부 40 vs 실제 80"으로 표시 → 추적 결과 **현금장부 수기 행 2개가
  소프트 삭제(상태 '삭제') 상태인데 buildDayClose_ 만 세고 있었다**. 현금장부 본체(getCashLedgerAdmin)는
  '삭제'를 제외하는데 마감 대조는 '취소'만 걸렀던 비대칭.
- 수리: `/취소|삭제/` 로 동일 규칙 적용. 검증: 9/2 현금 실제 80→0(장부 40, 유령 행 사라짐), 카드 110 일치.
- 함께 추가: **현금 수기 행이 자동 파생(예약·인화 현금 수납)과 같은 날·금액·이름이면 경고** — 현금장부는
  중복 제거를 하지 않아 수기로 또 적으면 시재가 두 번 잡힌다(이번 건도 그래서 지워져 있었다).
- 부수 발견(운영 세션 몫): 8/29 마감 대조에 실제 불일치 2건 — SumUp 이 장부보다 €35 많음, 계좌이체 €130 이
  은행 입금에 없음(은행 CSV 는 8/31까지 반영되어 있어 stale 아님).

### 프로필 Basic/Business 돌상·백일상 미포함 명시 (2026-09-05, @925 + 프런트)

- 사장님 질문 "프로필 비즈니스에 돌상 셋팅 어렵다는 안내가 나가나?" → 실측: 성공 화면(제출 직후 카드)엔
  "프로페셔널 이상부터" 안내가 있었지만, **메일은 프로필 경로(연령→영유아→백일/돌)를 아기 예약으로 못 봐서
  아기 안내 블록 자체가 빠졌고**, 상품 고르는 단계엔 언급이 없었다.
- 수리: `_guideSurveyKeys_`(babyType·ageGroup 으로 'baby' 키 보강) + `_dolIncluded_`(성공 화면과 같은 규칙:
  스튜디오·스냅 전 상품, 프로필은 pp 만) → 접수·확정 메일 아기 블록이 프로필 Basic/Business 면
  "⚠️ 돌상/백일상 셋팅은 선택하신 상품(프로필 Business)에는 포함되지 않습니다 — 프로페셔널(€130) 이상 무료.
  회신으로 상품 변경 요청 가능" (3개국어). 예약 페이지 프로필 Basic/Business 상세에 같은 취지 한 줄
  (`getDolHintHtml`, 협력업체 힌트 자리). 상품설정 시트 문구는 불변.

### 오늘촬영 앱 '마감 대조' 시트 (2026-09-05, 앱 재빌드 — 서버 변경 없음)

- 서버 `day-close`(하루 마감 대조, 조회 전용)가 CLI 에만 있고 어드민·앱 어디에도 화면이 없었다 →
  보드 하단 「마감 대조」 버튼 → 시트: 장부 수납·실제 거래·받을 예정 합계, **수단별 대조**(카드↔SumUp,
  계좌↔은행 — 은행 CSV 미반영 날은 '대조 불가' 표시, 현금은 서랍 실물), 서버 경고, 미수 잔금·장부 수납·
  실제 거래 목록. 촬영 0건인 날도 버튼 노출(픽업·추가금 수납이 있을 수 있다). 응답은 느슨한 딕셔너리로
  읽어 키 하나가 없어도 화면이 죽지 않는다.
- 첫 대조(9/2)가 바로 잡은 것: 현금 장부 40€(Wang 추가금) vs 현금장부 수기 입금 80€ — '왕혜연 40' 두 줄
  → 현금장부 중복 의심(운영 세션 확인 대상). SumUp 110 VISA 는 정혜미 잔금으로 연결 확인.
...
- `select-extra-unpay`(@923, ✏️): 수납 되돌리기 — 현주현 행 9 를 13€ 미수로 환원(사장님 확인: 재방문할인 차액 차감
  후 잔액 미수령). 8/26 현금 13€ 가 현금장부에서 빠지므로 8월 시재 −13€ (8월 마감 미확정이라 drift 아님).
