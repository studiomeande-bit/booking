...
### 9. 환불 3경로 — 검증 결과 (2026-08-28)

**② 굿샤인으로 결제된 예약의 환불 = 바우처 복원 — 자동화 완료, 라이브 검증.**
`booking-refund` + `method:'gutschein'` (취소된 예약 전용). 실측: 적용→예약취소→복원 후 굿샤인이
`판매완료·미사용·연결행 0` 으로 되돌아왔고, **판매 과세는 발행일 그대로 불변**(SPV — 사용만 되돌리므로
세무상 가장 단순), **현금 유출 0**. 예약이 이미 삭제된 고아 상태는 `gutschein-cancel force+사유` 로 정리.

**① 판매한 굿샤인 자체의 현금 환불 — 자동화 없음(의도적). 수기 절차:**
PDF 약관에 `Keine Barauszahlung` 이 인쇄돼 있어 현금 환불은 계약상 원칙적으로 배제된다.
법적으로 물러야 하는 예외(§ 812 등)가 생기면:
- **같은 해**: `gutschein-cancel`(사용 전) 또는 `force+사유`(사용 후) → 장부에서 판매가 빠진다.
  분기가 이미 신고된 뒤라면 월마감 drift 감시가 잡고, 분기 정정 금지 정책(2026-07-18)대로
  **연간신고에서 일괄 정산**된다.
- **해를 넘긴 환불**(작년 판매분): **취소하지 말 것** — 취소하면 이미 신고된 작년 EÜR·연간 USt 가
  소급으로 틀어진다. 대신 굿샤인은 상태 유지 + 관리메모 기록, 환불금은 **환불일 기준 수기 지출**
  (`cash-add` 출금 또는 `expense-add`)로 기장한다. 이월조정 트래커에 한 줄 남길 것.

**③ 만료 후 청구(§ 812 가액반환)** — 기한이 지나도 고객이 가액반환(일실이익 공제)을 청구할 수 있다.
만료를 공짜 수익으로 계산하지 말 것. 실제 청구가 오면 ①의 절차를 따른다.
